...
This case study illustrates the feasibility of reference based analysis of single-cell data to interpret cell states beyond those represented in the reference map. Additionally, we will show how, upon encountering novel cell states, a reference map can be updated to incorporate the new diversity.

For this example, we will use a dataset of CD4+ T cells responding to MC38 tumors (all T cells are specific for the GP66 epitope), both from the tumor (TILs) and from lymphnodes (LN) of two mice. The data are available at [GEO - GSE200635](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE200635). We will project these T cells on a reference map of LCMV virus-specific CD4+ T cells, and determine the degree of correspondence of the T cells from the two disease models.
...
```{r, message=FALSE, warning=F, results=FALSE, eval=F}
gse <- getGEO(geo_acc)
...
One may want to perform standard quality checks (e.g. number of UMIs, percentage of mitochondrial genes etc.), but for the sake of simplicity we will keep all cells in this case study.
...
query.projected <- make.projection(query.list, ref=ref, anchor_coverage =  0.3)
```
...
```
While T cells from the lymphnode are mostly of the follicular helper lineage, tumor-infiltrating T cells are predicted to be mostly Tregs and T helpers. We can inspect a panel of markers genes for these subtype, to verify how well they match the reference profiles.
...
Indeed, tumor-specific T cells assigned to Th1 cluster have a much lower silhouette coefficient than other subtypes, suggesting that they fit poorly with the reference Th1 effectors.
...
a <- EnhancedVolcano(tab, lab = rownames(tab), x = 'avg_log2FC', y = 'p_val',
                     ...
                     subtitle = "TILs 1 vs. LCMV spleen",
                     drawConnectors = T, max.overlaps = 20)

a
```
Interestingly, several markers associated with Th2 cells are overexpressed in tumoral T helpers, e.g. Ccl1, Ccr8 and Igfbp7, while Th1 markers are downregulated (Ccl5, Ly6c2). This may indicate that helper T cells in the tumor have phenotype more similar to Th2 rather than the Th1 lineage.


# Recalculate map with novel state

We can recaulculate the low dimensional representation of the data to also account for the projected data. In this way, any novelty brought by the new dataset can contribute in determining the low dimensional space.
...
plot.projection(ref=merged, query=subset(merged, subset=ref_or_query=="query"), cols = palette, linesize=0.5) + 
  theme(aspect.ratio = 1) + ggtitle("Projected cells in recalculated reference space")
```
...
DimPlot(merged, reduction = "umap", group.by = "seurat_clusters", label = T) + theme(aspect.ratio=1) + theme(axis.ticks = element_blank(), axis.text = element_blank()) + NoLegend()
```
We can now annotate the new cluster of Th-like cells from the tumor, as well as the combined cluster of Tregs from the reference and the tumor. This constitutes an updated reference that you can use directly to project and interpret new data!
...
* M. Andreatta, Z. Sherman, A. Tjitropranoto, M. C. Kelly, T. Ciucci, S. J. Carmona **"A single-cell reference atlas delineates CD4+ T cell subtype-specific adaptation during acute and chronic viral infections"** bioRxiv 2021.09.20.458613; doi: https://doi.org/10.1101/2021.09.20.458613 
