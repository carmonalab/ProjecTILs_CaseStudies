...
remotes::install_github("carmonalab/ProjecTILs", ref="v0.6.0")
...
pl <- VlnPlot(query.merged , features = c("nFeature_RNA","nCount_RNA","percent.ribo","percent.mito"), ncol = 2, pt.size=0.001, combine=FALSE, split.by = "Tissue")
...
DimPlot(ref, reduction = "umap", label = TRUE, pt.size = 0.5, group.by = "functional.cluster", dims = c(2,1), cols = functional.cluster.colors) + 
  NoLegend() + theme(aspect.ratio = 1)  + scale_x_reverse() + scale_y_reverse() + ggtitle("Reference CD8 LCMV atlas")
...
The projection analysis showed that different tissues are composed of different proportions of broad T cell states. However, we may also ask whether there is variability between cells *within* specific reference T cell states. For instance, do effector (SLEC) cells from blood differential express genes compared to spleen SLEC cells? do terminally exhausted (Tex) cells from liver differ from Tex cells from spleen?
...
EnhancedVolcano(deg, lab = rownames(deg),x = 'avg_logFC', y = 'p_val', pCutoff = 10e-10, FCcutoff = 0.5, labSize = 5,
...
EnhancedVolcano(deg2, lab = rownames(deg2),x = 'avg_logFC', y = 'p_val', pCutoff = 10e-10, FCcutoff = 0.5, labSize = 5,
                legendPosition = 'none', drawConnectors = F,
...
* Sandu I, Cerletti D, Oetiker N, Borsa M, Wagen F, Spadafora I, Welten SP, Stolz U, Oxenius A, Claassen M. Landscape of exhausted virus-specific CD8 T cells in chronic LCMV infection. *Cell Reports.* **2020** Aug 25;32(8):108078.
